...
dat = filter(eu_makro, country %in% c("Germany", "France", "Netherlands"))
```
Hier waere ein guter Zeitpunkt das Addin 'Check Problemset' auszufuehren!
...
# enter your code here ...
reg1 <- lm(formula = unempl ~ gdp_growth,data = dat)
...
# enter your code here ...
reg1 <- lm(formula = bip ~ k, data = pwt)
summary(reg1)
```
...
# enter your code here ...
reg2 <- lm(formula = log_bip ~ log_k, data=pwt)
summary(reg2)
```
